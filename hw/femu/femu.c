...
void nvme_write_amplification(FemuCtrl *n, NvmeCmd *cmd) {
    printf("FEMU: nvme_write_amplification\n");
    struct ssd *ssd = n->ssd;

    float waf = 0.0;
    if (ssd->bytes_written_from_host > 0) {
        waf = 100.0 * (float)(ssd->bytes_written_from_host + ssd->bytes_written_during_gc) 
                   / ssd->bytes_written_from_host;
    }
    
    uint64_t waf_value = (uint64_t)(waf * 100);
    printf("FEMU: nvme_write_amplification - WAF: %.2f\n", waf);
    printf("FEMU: nvme_write_amplification - WAF_VAL: %ld\n", waf_value);
}
